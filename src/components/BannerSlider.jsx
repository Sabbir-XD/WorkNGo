  ...
  const slides = [
    {
      ...
      darkImage: "https://images.unsplash.com/photo-1573164713988-8665fc963095?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80",
      ...
      icon: <Users className="absolute -left-6 -top-6 w-32 h-32 text-emerald-800/20 dark:text-emerald-900/30" />,
    ...
    },
    {
      ...
      darkImage: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80",
      ...
      icon: <Briefcase className="absolute -right-8 -bottom-8 w-36 h-36 text-green-800/20 dark:text-green-900/30" />,
    ...
    },
    {
      ...
      darkImage: "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80",
      bgColor: "from-teal-900/80 to-cyan-900/80",
      darkBgColor: "from-teal-900/90 to-cyan-900/90",
      accentColor: "text-teal-100",
      buttonColor: "bg-teal-500 hover:bg-teal-600 dark:bg-teal-600 dark:hover:bg-teal-700",
      icon: <Zap className="absolute -left-10 -top-10 w-40 h-40 text-teal-800/20 dark:text-teal-900/30" />,
      ...
      return (
        <svg className={baseClasses} width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <pattern id="pattern-circles" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
            <circle cx="20" cy="20" r="1.5" fill="#10b981" className="dark:fill-emerald-500" opacity="0.4" />
          </pattern>
      ...
      return (
        <svg className={baseClasses} width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <pattern id="pattern-grid" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#059669" className="dark:stroke-green-500" strokeWidth="1" opacity="0.3" />
          </pattern>
      ...
      return (
        <svg className={baseClasses} width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <pattern id="pattern-diamonds" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M10 0L20 10L10 20L0 10Z" fill="#0d9488" className="dark:fill-teal-500" opacity="0.2" />
          </pattern>
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 hover:shadow-xl h-[400px] sm:h-[500px] lg:h-[600px] xl:h-[700px]">
              ...
              <div className="absolute inset-0">
                <img 
                  src={slide.image}
                  alt={slide.title}
                  className="w-full h-full object-cover object-center transition-all duration-1000 ease-out hover:scale-105 hidden dark:block"
                  loading="lazy"
                />
                <img 
                  src={slide.darkImage}
                  alt={slide.title}
                  className="w-full h-full object-cover object-center transition-all duration-1000 ease-out hover:scale-105 block dark:hidden"
                  loading="lazy"
                />
                <div className={`absolute inset-0 bg-gradient-to-r ${slide.bgColor} dark:${slide.darkBgColor}`} />
              </div>
              ...
              <div className="relative h-full flex flex-col justify-center p-6 sm:p-10 md:p-12 lg:p-16 max-w-2xl xl:max-w-3xl">
                ...
                <h2 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6 leading-tight ${slide.accentColor}`}>
                  {slide.title}
                </h2>
                <p className={`text-lg sm:text-xl md:text-2xl mb-6 sm:mb-8 max-w-lg ${slide.accentColor} opacity-90`}>
                  {slide.description}
                </p>
                <button className={`group relative z-10 inline-flex items-center gap-3 ${slide.buttonColor} text-white py-3 px-8 sm:py-4 sm:px-10 rounded-full font-semibold text-base sm:text-lg transition-all duration-300 hover:scale-105 shadow-md hover:shadow-lg`}>
                  {slide.buttonText}
              ...
              {/* Decorative elements */}
              <div className={`absolute top-1/4 -right-12 w-48 h-48 rounded-full ${slide.buttonColor.replace('hover:bg', 'bg').split(' ')[0]} opacity-20 blur-xl animate-pulse`} />
              <div className={`absolute bottom-8 left-8 w-16 h-16 rounded-full ${slide.buttonColor.replace('hover:bg', 'bg').split(' ')[0]} opacity-30 z-0 hidden sm:block`} />
            </div>
          </div>
        ))}
      </Slider>
